...
class MatrixDecomForRecSys(object):
    ...
    def bgd(self, P, Q, batch_size: int=8):
        ...
        *********************************
        ...
        for uid,iid,R_ui in self.train_data.itertuples(index=False):
            g1=self.reg_p*P[uid]
            g2=self.reg_q*Q[iid]
            for u,i,r_ui in self.train_data.sample(self.batch_size).itertuples(index=False):
                e_ui=r_ui-np.dot(P[u],Q[i])
                g1-=e_ui*Q[i]*(1/self.batch_size)
                g2-=e_ui*P[u]*(1/self.batch_size)
            P[uid]-=self.lr*g1
            Q[iid]-=self.lr*g2
        return P, Q
    ...
    def recommend(self,userid):
        ...
        max_item=user_rating.loc[user_rating['pred_rating']==user_rating['pred_rating'].max()]
        print("the recommended item for user %d is item %d, the predicted rating is %f"%(userid,max_item['movieId'],max_item['pred_rating']))
        # pred_rating_dataset = pd.read_csv("submit_results.csv")
        # pred_rating=pd.DataFrame(pred_rating_dataset)
        # userid_rating=pred_rating.loc[pred_rating['userId']==userid]
        # max_rating=userid_rating['pred_rating'].max()
        # max_item=userid_rating[userid_rating['pred_rating']==max_rating]
        # print("the recommended item for user %d is item %d, the predicted rating is %f"%(userid,max_item['movieId'],max_item['pred_rating']))